...
import g58399.chess.model.Pieces.Bishop;
import g58399.chess.model.Pieces.King;
import g58399.chess.model.Pieces.Knight;
import g58399.chess.model.Pieces.Pawn;
import g58399.chess.model.Pieces.Piece;
import g58399.chess.model.Pieces.Queen;
import g58399.chess.model.Pieces.Rook;
...
public class Game implements Model {
    ...
    public boolean isValidMove(Position oldPos, Position newPos) {
        ...
        if (!possibleMoves.contains(newPos)) {
            throw new IllegalArgumentException("The new position chosen is not correct" + newPos);
        }
    ...
    /**
     * @return La liste De toute les coups valides du joueur courant.
     */
    @Override
    public List<Move> getAllValidation() {
        List<Move> allPosValide = new ArrayList();
        //On recupere toute les positions des pieces du joueur courant. 
        List<Position> listeOccupedBy = board.getPositionOccupiedBy(getCurrentPlayer());
        for (Position OccupeBy : listeOccupedBy) {
            List<Position> posPossibleMovePiece = new ArrayList();
            // on ajoute les positions (ou la piece ou il peut se deplacer) pour chaque piece que le joueur courant possede
            posPossibleMovePiece.addAll(board.getPiece(OccupeBy).getPossibleMoves(OccupeBy, board));
            for (Position posPossibleMove : posPossibleMovePiece) {
                // Pour chacune de ces positions tu vas verifier si Le mouvement entre les deux est valide
                if (isValidMove(OccupeBy, posPossibleMove)) {
                    // Si oui tu crees le coup et tu le mets dans La liste de move
                    Move move = new Move(OccupeBy, posPossibleMove);
                    allPosValide.add(move);
                }
            }
        }
        return allPosValide;
